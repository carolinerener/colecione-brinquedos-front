...
export default function ProdutosPage() {
  ...
  function irParaPagina(pagina: number) {
    router.push(`/produtos?page=${pagina}`);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }

  // Decide qual imagem mostrar para cada produto
  function urlDaImagem(produto: Produto): string | null {
    if (produto.image) {
      // Produtos com imagem no banco: aponta para storage do Laravel
      return `${baseUrl}/storage/${produto.image}`;
    }
    // Fallback: produtos seed (1-5) usam imagens estáticas do public
